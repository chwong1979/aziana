...
- Version pointer: `package.json` = `0.6.1`
...
The dedicated `public/events-catering-sint-maarten.html` page now uses the same EN/NL/FR/ES
device preference through `public/catering-localized-copy.js`. Its locale documents own the full
catering message, inquiry wording and metadata as blocks rather than long-form English replacement
strings. Fixed contact details and menu/integration URLs continue to come from the existing HTML and
shared site data. `worker/catering-localized-copy.test.js` enforces page wiring and locale parity.
